...
class TestDeepMaskIntegration:
    ...
    def test_config(self):
        ...
        return {
            ...
            "pred_dir": Path(os.path.expanduser(os.environ.get("CI_TESTING_PRED_DIR", "~/test_output"))),
            "ref_dir": Path(os.path.expanduser(os.environ.get("CI_TESTING_REF_DIR", "tests/reference_data"))),
        }
    ...
    def brain_mask_paths(self, test_config):
        ...
        pred_mask = test_config["pred_dir"] / patient_id / f"{patient_id}_brain_mask_final.nii.gz"
        
        # Reference brain mask from deepFCD
        ref_mask = test_config["ref_dir"] / "segmentations" / patient_id / f"{patient_id}_brain_mask_final.nii.gz"
    ...
    def test_brain_mask_overlap_metrics(self, brain_mask_paths):
        ...
        if not ants.image_physical_space_consistency(pred_img, ref_img):
            ...
            pred_img = ants.resample_image_to_target(pred_img, ref_img, interp_type="nearestNeighbor")
        ...
        dice = (2 * intersection) / (pred_volume + ref_volume) if (pred_volume + ref_volume) > 0 else 0
        ...
        print(f"Brain mask overlap metrics:")
    ...
    def test_brain_mask_dimensions(self, brain_mask_paths):
        ...
        assert len(pred_img.shape) == 3, f"Expected 3D image, got shape: {pred_img.shape}"
        assert len(ref_img.shape) == 3, f"Expected 3D reference, got shape: {ref_img.shape}"
    ...
    def test_brain_mask_anatomical_consistency(self, brain_mask_paths):
        ...
        if num_components > 1:
            ...
            component_sizes = [(labeled_mask == i).sum() for i in range(1, num_components + 1)]
...
class TestDeepMaskOutputs:
    ...
    def test_output_files_created(self, test_config):
        ...
        patient_id = test_config["patient_id"]
        pred_dir = test_config["pred_dir"] / patient_id  # Output is in subject subdirectory
